...
``` {r import data}
# V4 OTUs
otu_table <- read.table("data/V4_otutable99.shared",
...
# V4 metadata
V4_meta <- read.csv("data/V4_meta.csv",
  header = TRUE, na.strings = "?", row.names = 1,
  fileEncoding = "latin1"
)
...
world_map <- ggplot() +
  geom_polygon(data = wd, aes(x = long, y = lat, group = group), fill = "ivory1", color = "darkgrey") +
...
## Part B, C (Ternary diagrams of ionic composition)

```{r, ternary plots}
# Adding Na+ and K+ together
V4_meta$Na_K <- V4_meta$Na + V4_meta$K

cation_tern <- ggtern(V4_meta, aes(Ca, Na_K, Mg)) +
...
anion_tern <- ggtern(V4_meta, aes(SO4, HCO3_CO3, Cl)) +
  geom_point(aes(fill = type, shape = Continent), size = 3, stroke = 1) +
  scale_fill_manual(values = cols) +
  scale_shape_manual(values = c(25, 24, 22, 21)) +
  labs(
...
## Figure 2
### Part A (Ordination with salinity)
...
plot(otu_mds_rot)
ordisurf(otu_mds_rot ~ V4_meta$Salinity, add = TRUE)
...
# plot with the NMDS
salinity_surf <- ggplot(data = contour_vals, aes(x = NMDS1, y = NMDS2, z = z)) +
  geom_contour(aes(colour = "grey"), linewidth = 1, color = "grey") +
  theme_bw()
salinity_surf

# create a df for the contour labels
labelz <- data.frame(x = c(-1.18, -0.74, -0.5, -0.03, 0.32, 0.63, 1.1, 1.35, 1.58, 1.75, 1.9, 2.05, 2.2, 2.24), y = c(-0.8, 0.75, 1.3, 1.57, 1.8, 1.9, 1.72, 1.65, 1.42, 1.28, 1.06, 0.94, 0.82, 0.19), z = NA, labels = c(
...
B <- salinity_surf +
  geom_point(
    data = data_scores, aes(x = NMDS1, y = NMDS2, fill = V4_meta$type, shape = cont),
    size = 4, stroke = 1.2
  ) +
...
ggsave("Fig2_A.png", B, device = "png", dpi = 400)
ggsave("Fig2_A.svg", B, device = "svg", dpi = 400)

```


### Part B (Ordination with fitted environmental variables)
...
V4_envfit <- nmds_v4_envfit + geom_segment(
  data = vec_df_p, aes(
    x = 0, xend = NMDS1 * 2,
  ...
  scale_colour_manual(values = c(
    Na = "#AA0000FF", K = "#AA0000FF", Ca = "#AA0000FF",
...
### Part C (Ordination with SIMPER)

```{r, ordination with SIMPER}

# perform a SIMPER test using PAST3 using Bray-Curtis dissimilarities
# vegan's simper function can only compare two groups
# PAST GUI statistical tool https://www.nhm.uio.no/english/research/resources/past/
# Past3.exe -> Import the otu_table, copy ionic composition type information as 'group'
# Select all -> Multivariate -> Tests -> SIMPER -> Bray-Curtis distance
# Export the results as tsv file and add taxa to OTUs contributing to top 20% of variance, creating a file "V4_simper20.txt"
...
taxa_otus <- read.table("data/V4_simper20.txt", header = TRUE, row.names = 1, sep = "\t")
taxa <- taxa_otus$name
...
## Figure 3
### Part A (Variance assessment with PERMANOVA test)
...
## standardize
env_compl <- decostand(env_compl, method = "standardize")
...
## standardize
ionic_compl <- decostand(ionic_compl, method = "standardize")

# ion selection
rda1_ionic <- rda(otu_hell_compl ~ 1, ionic_compl)
rda2_ionic <- rda(otu_hell_compl ~ ., ionic_compl)
step.both <- ordistep(rda1_ionic,
  scope = formula(rda2_ionic), direction = "both",
