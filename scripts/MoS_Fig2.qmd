...
# Sample metadata
meta <- read.table("../data/v4_v7_meta.txt", header = TRUE, dec = ",", na.strings = "?", row.names = 1,
...
otutax <- read.table("../data/otus.98.80.80.taxonomy_V4_v5.taxonomy", header=TRUE,
                     row.names=2, sep="\t")
...
inditree <- read.tree("otu.rep.mod.pick.tre")
...
inner_nodes <- data.frame(node=c(675, 794, 906, 767, 838, 507, lineage=c("betI", "Frankiales", "Verrucomicrobiales", "Gemmatimonadota", "Nitriliruptorales", "Cytophagales")))
